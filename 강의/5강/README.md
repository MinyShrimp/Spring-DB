...
* 언체크 예외는 말 그대로 **컴파일러가 예외를 체크하지 않는다**는 뜻이다.
...
* **리포지토리**는 DB에 접근해서 데이터를 저장하고 관리한다. 여기서는 `SQLException`체크 예외를 던진다.
* **NetworkClient**는 외부 네트워크에 접속해서 어떤 기능을 처리하는 객체이다.
    * 여기서는 `ConnectException`체크 예외를 던진다.
* **서비스**는 리포지토리와 `NetworkClient`를 둘다 호출한다.
...
그래서 `SQLException`이 아니라 예를 들어서 `JPAException`으로 예외가 변경된다면 어떻게 될까?
...
* JDBC -> JPA 같이 기술로 변경하면 예외도 함께 변경해야한다. 그리고 해당 예외를 던지는 모든 다음 부분도 함께 변경해야 한다.
...
```
21:57:18.347 [main] INFO hello.springdb1.exception.basic.UnCheckedAppTest - ex
hello.springdb1.exception.basic.UnCheckedAppTest$RuntimeSQLException: java.sql.SQLException: ex
	at hello.springdb1.exception.basic.UnCheckedAppTest$Repository.call(UnCheckedAppTest.java:75)
	at hello.springdb1.exception.basic.UnCheckedAppTest$Service.logic(UnCheckedAppTest.java:50)
	at hello.springdb1.exception.basic.UnCheckedAppTest$Controller.request(UnCheckedAppTest.java:37)
	at hello.springdb1.exception.basic.UnCheckedAppTest.printEx(UnCheckedAppTest.java:23)
	... 
Caused by: java.sql.SQLException: ex
	at hello.springdb1.exception.basic.UnCheckedAppTest$Repository.runSQL(UnCheckedAppTest.java:81)
	at hello.springdb1.exception.basic.UnCheckedAppTest$Repository.call(UnCheckedAppTest.java:73)
...
```
hello.springdb1.exception.basic.UnCheckedAppTest$RuntimeSQLException: null
	at hello.springdb1.exception.basic.UnCheckedAppTest$Repository.call(UnCheckedAppTest.java:75)
	at hello.springdb1.exception.basic.UnCheckedAppTest$Service.logic(UnCheckedAppTest.java:50)
	at hello.springdb1.exception.basic.UnCheckedAppTest$Controller.request(UnCheckedAppTest.java:37)
	at hello.springdb1.exception.basic.UnCheckedAppTest.printEx(UnCheckedAppTest.java:23)
